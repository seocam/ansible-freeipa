...
def main():
        ...
        if options.ip_addresses is not None:
            for value in options.ip_addresses:
                try:
                    ...
                    raise ValueError(
                        "invalid IP address {0}: {1}".format(value, e)
                    )
        ...
        if options.ca_cert_files is not None:
            for value in options.ca_cert_files:
                ...
                if not os.path.isabs(value):
                    raise ValueError(
                        "'%s' is not an absolute file path" % value
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        "'%s' is not a valid certificate file" % value
                    )
    ...
    try:
        ...
        try:
            gssapi.Credentials(
                store={"keytab": paths.KRB5_KEYTAB}, usage="accept",
            )
        ...
        if (
            ...
            logger.warning(
                "Using existing certificate '%s'.", paths.IPA_CA_CRT
            )
        ...
        if ret == ipadiscovery.NOT_IPA_SERVER:
            ...
            raise ScriptError(
                "%s is not an IPA v2 Server." % cli_server[0],
                rval=CLIENT_INSTALL_ERROR,
            )
