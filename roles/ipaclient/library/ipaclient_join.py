...
def main():
    ...
    try:
        ...
        if principal is not None:
            ...
            if admin_keytab:
                ...
                if not os.path.exists(admin_keytab):
                    module.fail_json(
                        msg="Keytab file could not be found: %s" % admin_keytab
                    )
                try:
                    kinit_keytab(
                        principal,
                ...
                try:
                    kinit_password(
                        principal, password, ccache_name, config=krb_name
                    )
        ...
        if already_joined and not os.path.exists(paths.KRB5_KEYTAB):
            module.fail_json(
                msg="krb5.keytab missing! Retry with "
