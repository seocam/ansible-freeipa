...
def get_ipa_conf():
    ...
    return dict(basedn=basedn, realm=realm, domain=domain)
...
def main():
    ...
    facts = dict(
        packages=dict(ipalib=HAS_IPALIB, ipaserver=HAS_IPASERVER,),
        configured=dict(
            client=False,
            server=False,
            dns=False,
    ...
    module.exit_json(changed=False, ansible_facts=dict(ipa=facts))
