...
def main():
    ansible_module = AnsibleModule(
        argument_spec=dict(
            ...
            secondary_rid_base=dict(
                required=False, type="int", default=100000000
            ),
            # additional
    ...
    try:
        ...
        replman = ReplicationManager(
            config.realm_name, config.master_host_name, None
        )
        ...
        if ca_host is not None:
            ...
            if options.setup_ca:
                msg = (
                    "The remote master does not have a CA "
        ...
        if ca_enabled:
            ...
            ansible_log.debug(
                "  ca.external_cert_file=%s" % repr(ca.external_cert_file)
            )
            ansible_log.debug(
                "  ca.external_ca_file=%s" % repr(ca.external_ca_file)
            )
        ...
        if options.setup_dns:
            dns.install_check(
                False, remote_api, True, options, config.host_name
            )
